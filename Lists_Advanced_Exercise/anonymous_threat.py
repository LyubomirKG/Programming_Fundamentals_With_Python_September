def merge(command_list: list, entered_single: list) -> list:
    # Set boundaries and ensure they are within the list range
    start_index = max(0, int(command_list[1]))
    end_index = min(int(command_list[2]), len(entered_single) - 1)
    
    # If the start is after the end, no merging is possible
    if start_index > end_index:
        return entered_single
        
    # Join the elements in the range into a single string
    merge_data = "".join((entered_single[start_index:end_index + 1]))
    # Replace the range with the merged string
...
def divide(command_list: list, entered_single: list) -> list:
    ...
    # If there is a remainder, merge the last parts together
    if last_partition != 0:
...
# Initial input of the data list
single_line = input().split()

# Process commands until "3:1" is received
while True:
    command = input().split()
    if command[0] == "3:1":
        break
...
# Print the final result joined by space
print(*single_line)
